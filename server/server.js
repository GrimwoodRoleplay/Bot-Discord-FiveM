...
on("playerJoining", (oldId) => {
    ...
    if (z.bot.isRolePresent(member, z.config.StaffChatRoleIds)) {
        ExecuteCommand(`add_principal "player.${source}" group.botdiscordstaff`);
    }
});
...
if (z.config.EnableStaffChatForwarding) {
    RegisterCommand("staff", (source, args, raw) => {
        if (!IsPlayerAceAllowed(source, "botdiscord.staffchat")) return;
    ...
    RegisterCommand("stafftoggle", (source, args, raw) => {
        if (IsPlayerAceAllowed(source, "botdiscord.staffchat")) {
            ExecuteCommand(`remove_principal "player.${source}" group.botdiscordstaff`);
